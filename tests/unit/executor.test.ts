...
describe('execute', () => {
  ...
  it('does not leak SECRET_KEY or other non-allowlisted env vars', async () => {
    ...
    try {
      const result = await execute({
        cmd: 'node',
        args: ['-e', 'process.stdout.write(process.env.SECRET_KEY || "NOT_SET")'],
        cwd: process.cwd(),
      }, 5000);
  ...
  it('merges invocation.env into child env', async () => {
    const result = await execute({
      cmd: 'node',
      args: ['-e', 'process.stdout.write(process.env.MY_TOOL_VAR || "MISSING")'],
  ...
  it('always sets CI=true and NO_COLOR=1', async () => {
    const result = await execute({
      cmd: 'node',
      args: ['-e', 'process.stdout.write(`${process.env.CI}:${process.env.NO_COLOR}`)'],
      cwd: process.cwd(),
    }, 5000);
