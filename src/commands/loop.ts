      ...
      async (
        ...
        if (preset) {
          ...
          if (opts.dryRun) {
            ...
            // Phase 1: Discovery
            info(`Running discovery phase with ${discoveryToolId}...`);
            ...
            info(`Writing execution prompt with ${discoveryToolId}...`);
        ...
        try {
          const loopResult = await runLoop({
            ...
            onConvergence: (round, ratio) => {
              info(
                `Convergence detected at round ${round} (ratio: ${ratio.toFixed(2)} < ${convergenceThreshold})`,
              );
            },
          });
