...
import { formatRunSummary, formatDryRun } from '../ui/output.js';
...
export function registerRunCommand(program: Command): void {
  program
    ...
    .option('--context <paths>', 'Gather context from paths (comma-separated, or "." for git diff)')
    .option('--read-only <level>', 'Read-only policy: strict, best-effort, off', 'best-effort')
    ...
    .action(async (promptArg: string | undefined, opts: {
      ...
      if (!readOnlyPolicy) {
        error(`Invalid --read-only value "${opts.readOnly}". Must be: strict, best-effort, or off.`);
        process.exitCode = 1;
        return;
      }
      ...
      if (opts.file) {
        ...
        const context = opts.context
          ? gatherContext(cwd, opts.context === '.' ? [] : opts.context.split(','), config.defaults.maxContextKb)
          : undefined;
        ...
        if (process.stdin.isTTY) {
          error('No prompt provided. Pass as argument, use -f <file>, or pipe via stdin.');
          process.exitCode = 1;
          return;
        }
      ...
      if (opts.dryRun) {
        ...
        const invocations = toolIds.map(id => {
          ...
          return { toolId: id, model: toolConfig.defaultModel, cmd: inv.cmd, args: inv.args };
        });
      ...
      const display = new ProgressDisplay(
        toolIds.map(id => ({ toolId: id, model: config.tools[id].defaultModel })),
        outputDir,
      );
      ...
      const reports = await dispatch({
        ...
        onProgress: (event) => {
          ...
          if (event.event === 'completed') display.complete(event.toolId, event.report!);
        },
      });
      ...
      const manifest: RunManifest = {
        ...
        slug,
        prompt: promptArg || (opts.file ? `file:${basename(opts.file)}` : 'stdin'),
        promptSource,
        readOnlyPolicy,
        tools: reports,
      };
      ...
      safeWriteFile(resolve(outputDir, 'run.json'), JSON.stringify(manifest, null, 2));
